...
public struct StudyDesigner: Sendable {
    ...
    public func analyse(protocol studyProtocol: StudyProtocol) -> StudyAnalysis {
        ...
        let blockers = consentViolations.filter { $0.severity == .blocker }
            + federatedIssues.filter { $0.severity == .critical }

        let warnings = consentViolations.filter { $0.severity == .warning }
            + federatedIssues.filter { $0.severity == .warning }

        let readiness: StudyAnalysis.ReadinessLevel
        if !blockers.isEmpty || !dpPlan.isWithinBudget {
            readiness = .blocked
        } else if !warnings.isEmpty || !dpPlan.recommendations.isEmpty {
